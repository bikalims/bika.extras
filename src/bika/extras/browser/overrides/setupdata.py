...
class Analysis_Specifications(WorksheetImporter):
    def resolve_service(self, row):
        ...
        service = bsc(
            portal_type="AnalysisService", title=safe_unicode(row["service"])
        )
        if not service:
            service = bsc(
    ...
    def Import(self):
        ...
        for row in self.get_rows(3):
            ...
            supplier = self.get_object(
                pc, "Supplier", getName=row.get("Supplier", "")
            )
            ...
            if row.get("Photo", None):
                path = resource_filename(
                    ...
                    "setupdata/%s/%s" % (self.dataset_name, row["Photo"]),
                )
                try:
            ...
            if row.get("InstalationCertificate", None):
                path = resource_filename(
                    self.dataset_project,
                    "setupdata/%s/%s"
                    % (self.dataset_name, row["InstalationCertificate"]),
                )
                try:
                    file_data = read_file(path)
...
class Methods(WorksheetImporter):
    def load_instrument_methods(self):
        ...
        for i, row in enumerate(self.get_rows(3, worksheet=worksheet)):
            if not row.get("Instrument_title", "") or not row.get(
                "Method_title", ""
            ):
                continue
            ...
            instrument = self.get_object(
                bsc, "Instrument", title=row["Instrument_title"]
            )
            if instrument:
            ...
            if row["title"]:
                ...
                for methods in methods_brains:
                    if (
                        ...
                        and methods.getObject.get("MethodID", "")
                        == obj["MethodID"]
                    ):
                        obj.edit(MethodID="")
    ...
    def load_result_options(self):
        ...
        for row in self.get_rows(3, worksheet=worksheet):
            service = self.get_object(
                bsc, "AnalysisService", row.get("Service_title")
            )
            if not service:
                return
    ...
    def Import(self):
        ...
        for row in self.get_rows(3):
            ...
            MTA = {
                ...
                "minutes": self.to_int(
                    row.get("MaxTimeAllowed_minutes", 0), 0
                ),
            }
            ...
            container = self.get_object(
                bsc, "Container", row.get("Container_title")
            )
            ...
            defaultmethod = self.get_object(
                pc, "Method", row.get("DefaultMethod_title")
            )
            ...
            allowmanualentry = (
                True
                if not allowinstrentry
                else row.get("ManualEntryOfResults", True)
            )
            ...
            obj.edit(
                ...
                ExponentialFormatPrecision=str(
                    self.to_int(row.get("ExponentialFormatPrecision", 7), 7)
                ),
                LowerDetectionLimit="%06f"
                % self.to_float(row.get("LowerDetectionLimit", "0.0"), 0),
                UpperDetectionLimit="%06f"
                % self.to_float(
                    row.get("UpperDetectionLimit", "1000000000.0"),
                    1000000000.0,
                ),
                ...
                InterimFields=hasattr(self, "service_interims")
                and self.service_interims.get(row["title"], [])
                or [],
...
class AR_Templates(WorksheetImporter):
    def load_artemplate_analyses(self):
        ...
        for row in self.get_rows(3, worksheet=worksheet):
            ...
            service = self.get_object(
                bsc, "AnalysisService", row.get("service_uid")
            )
            if not service:
                continue
            ...
            self.artemplate_analyses[row["ARTemplate"]].append(
                {"service_uid": service.UID(), "partition": row["partition"]}
            )
    ...
    def load_artemplate_partitions(self):
        ...
        for row in self.get_rows(3, worksheet=worksheet):
            ...
            container = self.get_object(bsc, "Container", row.get("container"))
            preservation = self.get_object(
                bsc, "Preservation", row.get("preservation")
            )
    ...
    def Import(self):
        ...
        for row in self.get_rows(3):
            ...
            if row["title"] in self.artemplate_partitions:
                ...
                partitions = [
                    {"part_id": "part-1", "container": "", "preservation": ""}
                ]

            if client_title == "lab":
                folder = self.context.bika_setup.bika_artemplates
            else:
                folder = pc(portal_type="Client", getName=client_title)[
                    0
                ].getObject()

            sampletype = self.get_object(
                bsc, "SampleType", row.get("SampleType_title")
            )
            samplepoint = self.get_object(
                bsc, "SamplePoint", row.get("SamplePoint_title")
            )
    ...
    def load_reference_definition_results(self):
        ...
        for row in self.get_rows(3, worksheet=worksheet):
            ...
            service = self.get_object(
                ...
                "AnalysisService",
                row.get("service"),
                **{"getKeyword": row.get("Keyword")}
            )
            if not service:
                continue
            self.results[row["ReferenceDefinition_title"]].append(
                {
                    "uid": service.UID(),
...
class Worksheet_Templates(WorksheetImporter):
    def load_wst_layouts(self):
        ...
        if not worksheet:
            return
        for row in self.get_rows(3, worksheet=worksheet):
            if row["WorksheetTemplate_title"] not in self.wst_layouts.keys():
                self.wst_layouts[row["WorksheetTemplate_title"]] = []
            self.wst_layouts[row["WorksheetTemplate_title"]].append(
                {
                    "pos": row["pos"],
                    "type": row["type"],
    ...
    def load_wst_services(self):
        ...
        for row in self.get_rows(3, worksheet=worksheet):
            ...
            if not service:
                continue
            self.wst_services[row["WorksheetTemplate_title"]].append(
                service.UID()
            )
